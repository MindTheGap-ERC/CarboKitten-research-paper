...
``` {.make .figure target=tools/xml-gen/fig/pythagoras.png}
Pythagorean tree
---
...
``` {.scheme file=tools/xml-gen/example/pythagoras.scm}
(import (rnrs (6))
        (srfi srfi-13))
...
(define style-sheet
  (strip-css-comments
    (call-with-input-file "tools/xml-gen/example/pythagoras.css" get-string-all)))
...
``` {.css file=tools/xml-gen/example/pythagoras.css}
g.leaf circle {
   fill: #ffcccc;
}
...
``` {.scheme file=tools/xml-gen/xml-gen.scm}
(import (rnrs)
        (rnrs eval)
        (ice-9 match)
        (ice-9 format))
